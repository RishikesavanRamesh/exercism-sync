...
function exchange_rate_with_spread(exchange_rate, spread)
    (100+spread)*exchange_rate/100
end

function exchangeable_value(budget, exchange_rate, spread, denomination)
    value = exchange_money(budget, exchange_rate_with_spread(exchange_rate,spread))
